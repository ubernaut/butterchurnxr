...
pixels are downsampled and assembled with
[deno_tui](https://github.com/ubernaut/deno_tui)'s block grid. Its microphone
PCM receives the same 4x gain as the parent demo. The parent demo's
`butterchurn-presets@3.0.0-beta.4` base and extra packs are merged and sorted,
providing the same 293 presets.
...
The required `deno_tui` revision is pinned by immutable Git commit in
`deno.jsonc`, with its source integrity recorded in `deno.lock`; a sibling
`deno_tui` checkout is not required. The first run needs network access to
populate Deno's global dependency cache.
